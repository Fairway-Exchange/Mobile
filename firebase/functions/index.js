...
const kStripeTestSecretKey =
  "sk_test_51Ps86cG5JZDGZYXLzhZC2OoT8UMf8e5Ico2rYcFxeuFn5sXybur7aOiLRU3vMEVGxz16SF0xDtEx8Pgr04m6o0cK00ucfIDKbR";
...
exports.onUserDeleted = functions.auth.user().onDelete(async (user) => {
  ...
  await firestore
    .collection("listings")
    .where("sellerRef", "==", userRef)
    .get()
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(`Deleting document ${doc.id} from collection listings`);
        await doc.ref.delete();
      }
    });
  await firestore
    .collection("bids")
    .where("bidderID", "==", userRef)
    .get()
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(`Deleting document ${doc.id} from collection bids`);
        await doc.ref.delete();
      }
    });
  await firestore
    .collection("productReviews")
    .where("reviewerId", "==", userRef)
    .get()
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(
          `Deleting document ${doc.id} from collection productReviews`,
        );
        await doc.ref.delete();
      }
    });
  await firestore
    ...
    .then(async (querySnapshot) => {
      for (var doc of querySnapshot.docs) {
        console.log(
          `Deleting document ${doc.id} from collection sellerReviews`,
        );
        await doc.ref.delete();
      }
    });
